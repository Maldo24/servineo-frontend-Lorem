...
export default function TopMenu() {
        ...
        user?.role === 'fixer'
            ? [
                ...
                },
                {
                    ...
                    onClick: () => sessionStorage.setItem('roluser', 'fixer')
    ...
    return (
        <>
            {/* DESKTOP HEADER */}
            <header
                className={`hidden lg:block fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-md' : 'bg-white'
                ...
                <nav className='fixed bottom-0 left-0 right-0 h-16 border-t border-gray-200 bg-white/95 backdrop-blur-sm flex justify-around items-center z-50'>
                    {navItems.map((item) => (
                        <button
                            ...
                            className={`flex flex-col items-center text-[11px] px-1 py-1 ${currentPath === item.href
